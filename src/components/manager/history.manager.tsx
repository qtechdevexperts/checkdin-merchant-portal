...
const CouponHistory: React.FC = () => {
  ...
  const fetchCoupens = async () => {
    ...
    const res = await fetch(
      `https://api.chekdin.com/api/v1/coupon/redeemed-coupons?coupon_id=${id}`,
      {
        headers: {
  ...
  const deleteItem = async (UID: string | number, CID: string | number) => {
    ...
    try {
      let response = await fetch(
        `https://www.api.chekding.com/api/v1/coupon/undo-coupon-redemption`,
        requestBody
      );
      console.log(response);
          ...
          <Table hover className="user-table align-items-center">
            <thead>
              <tr>
                ...
                <th className="border-bottom">Redemption Date</th>
              </tr>
            </thead>
            <tbody>
              {data.length > 0 ? (
