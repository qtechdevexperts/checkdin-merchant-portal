...
import { AUTH_SIGNIN_URL, AUTH_SIGNUP_URL, AUTH_SIGNUP_CONFIRMATION_URL, AUTH_FORGOTPASSWORD_URL, AUTH_CONFIRMFORGOTPASSWORD_URL } from "../constants.js";
...
export const auth_login = async (username: string, password: string) => 
{
    ...
    else
    {
      ...
      const accessToken = JSON.stringify( parsedRes.AuthenticationResult.AccessToken );
      const refreshToken = JSON.stringify( parsedRes.AuthenticationResult.RefreshToken );
...
export const forgotPasswordConfirm = async (code: string, password: string, username: string) => {
